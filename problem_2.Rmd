...
CVgeneric <- function(classifier, data, k, loss, splitter=split2){
  ...
  for (i in 1:k){
    ...
    #qda
    else if(identical(classifier, "qda")){
      formula <- as.formula(cloud_label ~ NDAI + CORR + SD)
...
tiny_image_list <- list(image1[1:10000,],image2[1:10000,],image3[1:10000,])


#knn
CVgeneric(classifier="knn", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split1)
CVgeneric(classifier="knn", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split2)

#glm
CVgeneric(classifier="glm", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split1)
CVgeneric(classifier="glm", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split2)
    
#lda
CVgeneric(classifier="lda", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split1)
CVgeneric(classifier="lda", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split2)
 
#qda
CVgeneric(classifier="qda", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split1)
CVgeneric(classifier="qda", data=tiny_image_list, k=5, loss=zero_one_loss, splitter=split2)
...
test_split <- split2(tiny_image_frame, spec=c(train=0.9, test=0.1))
...
# split2 needs a list
# now split 2 isnt working, based on the changes we made to split1, and i have no idea why or how to fix it...
set.seed(123456)
...
folds <- createFolds(split_train$cloud_label, k = 10)
loss <- rep(0,10)
for(i in 1:10){
    val_dat1 <- split_train[folds[[i]],]
    train_dat1 <- split_train[-folds[[i]],]

    class <- knn(train_dat1[,4:6], val_dat1[,4:6],
    train_dat1$cloud_label, 10)
    
    loss[i] <- zero_one_loss(val_dat1$cloud_label, class)
}
...
CVgeneric2 <- function(classifier, data, K, loss, splitter=split1, featstrain, labeltrain){
  set.seed(12345678)
  split_data <- splitter(data, spec=c(train=0.9,test=0.1))
  datatrain <- split_data$train
  #still the error
...
CVgeneric2("knn",tiny_image_list,10,loss=zero_one_loss,split2,
