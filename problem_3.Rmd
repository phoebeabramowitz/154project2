...
imagestrain <- mutate(timagestrain, cl=ifelse(cloud_label==1, 1, 0))
imagestest <- mutate(timagestest, cl=ifelse(cloud_label==1, 1, 0))
>>>>>>> parent of 24fd0fb... Update problem_3.Rmd
```
...
```
#### LDA (Something is wrong here and in QDA)
```{r} 
#CV for both fold methods, across folds
...
zero_one_loss(datclassedlda, imagestest$cloud_label)
```

#### QDA
```{r}
CVmodel_accuracy("qda",timagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
CVmodel_accuracy("qda",imagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
#Transformed data has huigher CV accuracy 
model <- qda(cloud_label ~ NDAI + CORR +SD,timagestrain)
model.pred <- predict(model, timagestest)
datclassedqda <- model.pred$class
zero_one_loss(datclassedqda, imagestest$cloud_label)
```
