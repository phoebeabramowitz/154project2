...
```{r}
# data is just the training dataset
CVmodel_accuracy <- function(classifier, data, k, loss){
...
CVmodel_accuracy(lda, imagestrain, 5, zero_one_loss)
```

```{r}
# library(caret)
# 
# model_accuracy <- function(classifier, data, k, loss){
#   
#   folds <- createFolds(data$cloud_label, k = k)
#   fold_loss <- rep(0,k)
#   
...
#     train_dat <- data[-folds[[i]],]
#     
#     mod_fit <- train(cloud_label ~ NDAI + CORR + SD, data = data,
#                      method=classifier )
#   }
# }
```
