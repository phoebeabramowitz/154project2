...
```{r}
#Same function but without the splitter
#Data should be just a training set(ex:timagestrain)
...
CVmodel_accuracy("glm",timagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")#First fold method
CVmodel_accuracy("glm",imagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
#Test Accuracy- Transformed images have better CV accuracy, so use second split on test data
timagestrain$cloud_label <- replace(timagestrain$cloud_label,timagestrain$cloud_label == -1, 0)
timagestest$cloud_label <- replace(timagestest$cloud_label,timagestest$cloud_label == -1, 0)
model <- glm("cloud_label ~ NDAI + CORR +SD", data=timagestrain, family="binomial")
model.pred <- predict(model, timagestest, type="response")
datclassed <- rep(0, length(model.pred))
datclassed[model.pred >= 0.5] = 1
zero_one_loss(datclassed, timagestest$cloud_label)
...
```{r} 
#CV for both fold methods, across folds
t1 <- CVmodel_accuracy("knn",timagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")#First fold method
t2 <- CVmodel_accuracy("knn",imagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
mean(t1-t2)
#Test Accuracy- Untransformed images have better slightly better CV and test accuracy, so use first split on test data
datclassed <- knn(imagestrain[,4:6], imagestest[,4:6],imagestrain$cloud_label,k=10)
zero_one_loss(datclassed, imagestest$cloud_label)
...
```{r}
# trying boosting, since ill want to do it in part 4. once the CVgeneric is up and running, we can include this as an else is..


# split1
boost1 <- gbm(cloud_label ~ NDAI + CORR + SD,
              data = log_imagestrain,
              distribution = "gaussian")

boost1.pred <- predict(boost1, log_imagesval, n.trees = 100)
boost1_pred <- rep(0, length(boost1.pred))
boost1_pred[boost1.pred >= 0.5] = 1

mean(boost1_pred == log_imagesval$cloud_label)

# split2
boost2 <- gbm(cloud_label ~ NDAI + CORR + SD,
              data = tlog_imagestrain,
              distribution = "gaussian")

boost2.pred <- predict(boost2, tlog_imagesval, n.trees = 100)
boost2_pred <- rep(0, length(boost2.pred))
boost2_pred[boost2.pred >= 0.5] = 1

mean(boost2_pred == tlog_imagesval$cloud_label)

# trying adaboost split1
