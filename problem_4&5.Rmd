...
It's possible that our model will work more poorly on future data with expert labels, if there factors in that data that cause the cloudiness of a region to have a different relationship with our features than it has in the three images we trained our model on. Ideally, we would train our model on more images as they do in the paper to account for possible discrepancies between images and their respective polar regions. 
...
# pca_accuracy is the list of transformed pca.. so need to run it on untransformed and compare
```
