...
# (a) Do an in-depth analysis of a good classification model of your choice by showing some diagnostic plots or information related to convergence or parameter estimation.
  ...
  KNN outperformed the other classifiers we used, and it has a higher accuracy using the transformed data set. In part three, we only used NDAI, CORR, and SD to train our models, so in this section we explore KNN accuracy as K increases, while changing the training features. 
  We see an increase in model accuracy when adding more radiance angles as features, and a substantial increase in model accuracy when running PCA on the radiance angles. In part two of the project we deduced that PC1 of the radiance angles was not a good feature since the histograms of cloud label were largely overlapping. We see from this analysis however that it does improve the model accuracy for K-nearest neighbors significantly. 
...
```
The vast majority of the area in our model for all three images is error free. It's clear from the above images that errors are clustered in certain clouds or cloudless regions. Furthermore, those spatially clustered of errors are of the same type(i.e. there are clouds that have relatively high rates of being mislabelled as no cloud, a few small cloudless regions with high rates of being mislabelled as a cloud). 
...
ggplot(just_errors)+
  geom_histogram(aes(x=NDAI),fill="red")+
  aes(y=stat(count)/sum(stat(count))) + 
  scale_y_continuous(labels = scales::percent)+
  ylab("Percentage of Erroneous Points in this NDAI Bin")+
  ggtitle("Distribution of NDAI in Points Where our Model Gives an Error")

ggplot(data=imagestest,aes(x=NDAI))+
  geom_histogram(fill="blue")+
  aes(y=stat(count)/sum(stat(count))) + 
  scale_y_continuous(labels = scales::percent)+
  ylab("Percentage of Data Points in this NDAI Bin")+
  ggtitle("Distribution of NDAI in all Test Data")

ggplot(just_errors)+
  geom_histogram(aes(x=CORR),fill="violet")+
  aes(y=stat(count)/sum(stat(count))) + 
  scale_y_continuous(labels = scales::percent)+
  ylab("Percentage of Erroneous Points in this CORR Bin")+
  ggtitle("Distribution of CORR in Points Where our Model Gives an Error")

ggplot(data=imagestest,aes(x=CORR))+
  geom_histogram(fill="green")+
  aes(y=stat(count)/sum(stat(count))) + 
  scale_y_continuous(labels = scales::percent)+
  ylab("Percentage of Data Points in this CORR Bin")+
  ggtitle("Distribution of CORR in all Test Data")

ggplot(just_errors)+
  geom_histogram(aes(x=NDAI),fill="red",alpha=0.3)+
  aes(y=stat(count)/sum(stat(count))) + 
  geom_histogram(data=imagestest,aes(x=NDAI),fill="blue", alpha=0.3)+
  scale_y_continuous(labels = scales::percent)+
  ylab("Percentage of Points in this NDAI Bin")+
  ggtitle("Distribution of NDAI: Where our Model Gives an Error vs Overall")

ggplot(just_errors)+
  geom_histogram(aes(x=CORR),fill="red",alpha=0.3)+
  aes(y=stat(count)/sum(stat(count))) + 
  geom_histogram(data=imagestest,aes(x=CORR),fill="blue", alpha=0.3)+
  scale_y_continuous(labels = scales::percent)+
  ylab("Percentage of Points in this CORR Bin")+
  ggtitle("Distribution of CORR: Where our Model Gives an Error vs Overall")

ggplot(just_errors)+
  geom_histogram(aes(x=SD),fill="red",alpha=0.3)+
  aes(y=stat(count)/sum(stat(count))) + 
  geom_histogram(data=imagestest,aes(x=SD),fill="blue", alpha=0.3)+
  scale_y_continuous(labels = scales::percent)+
  ylab("Percentage of Erroneous Points in this SD Bin")+
  ggtitle("Distribution of SD: Where our Model Gives an Error vs Overall")
```
The distribution of NDAI values is substantially different among values where our model misclassifies than in the general test set. It would be valuable to consult with experts about the possible influence of NDAI on certain regions where our model has a relatively high rate failure. The distributions for SD and CORR are similar. 
...
Boosted regression trees is one classifier we used to improve model accuracy. Boosting fits the decision trees on a modified version of the original data set, growing them sequentially, and almost recursively. This boosted approach learns slowly, and uses errors from previous trees when training the next ones. In general this approach should perform well, but we see in our problem that is has a slightly lower test accuracy than KNN. 

It's possible that our model will work more poorly on future data with expert labels, if there exist factors in future data that cause the cloudiness of a region to have a different relationship with our features than it has in the three images we trained our model on. Ideally, we would train our model on more images as they do in the paper to account for possible discrepancies between images and their respective polar regions. 

# (d) Do your results in parts 4(a) and 4(b) change as you modify the way of splitting the data?

KNN accuracy (with PC1 as a feature) Comparison for both splits, first 10 Ks. Transformed is slightly better. 
...
After discovering the improved accuracy of adding PC1 of radiance angles as a training feature, we check to see whether this 


# (e) Write a paragraph for your conclusion.
Explicitly say which is better..boosting or KNN with PC1 of radiance angles as a feature, and talk about it. 
