...
datclassedknn <- knn(timagestrain[,4:6], timagestest[,4:6],timagestrain$cloud_label,k=10, prob=TRUE)
data.frame("KNN_test_accuracy"=zero_one_loss(datclassedknn, timagestest$cloud_label))
table(datclassedknn, timagestest$cloud_label)
```
...
```{r}
knnclasses <- knn(imagestrain[,4:6], imagestest[,4:6],imagestrain$cloud_label,k=8)
itesterrors <- mutate(imagestest, imagestest$cloud_label - knnclasses)
```
