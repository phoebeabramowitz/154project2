...
```{r}
#instead, create list of coords for every lower left corner of a super pixel and iterate through those lists(every combo)
#loop over every relevent point
#Raaz was saying to rbind a list 
#Use the grid thing to creat psuedo-random blocks, then take randomly from that
#Also good to take the average across features 
#ordered row-wise
#would it be good idea to take the mean of cloud_labels? for a measure of "cloudiness" 
#Should we delete every unlabelled point?
...
x1 <- seq(from=65, to=369, by=3)
y1 <- seq(from=2, to=383, by=3)
##Make empty dataframe for superpixels
images_columns <- c("y", "x","cloud_label","NDAI","SD","CORR","rad_DF","rad_CF",
                    "rad_BF","rad_AF","rad_AN" )
i1trans <-data.frame(matrix(ncol = 11, nrow = 0))
colnames(i1trans) <- images_columns
#start the new coordinates
supy=0
supx=0
for(i in x1){
  supx <- supx+1
  for(j in y1){
    supy <- supy+1
    #add a row to i1trans with average values of all 9 relevant pixels
    #at this point, there is at most one unique row with(x=i, y=j)
    pts <- image1 %>%filter((x==i | x==i+1 | x== i+2)&(y==j | y==j+1 | y== j+2))
    #from first problem, it's clear that the data is not a perfect rectangle
    if (nrow(pts)>0){
      new_row <- c(supy, supx,cloud_avg(pts$cloud_label),mean(pts$NDAI), mean(pts$SD),
                    mean(pts$CORR),mean(pts$rad_DF),mean(pts$rad_CF),mean(pts$rad_BF),
                    mean(pts$rad_AF),mean(pts$rad_AN))
      i1trans <- rbind(i1trans, new_row)
      # add_row(i1trans, y=supy, x=supx,cloud_label=cloud_avg(pts$cloud_label),
      #       NDAI=mean(pts$NDAI), SD=mean(pts$SD), CORR=mean(pts$CORR), 
      #       rad_DF=mean(pts$rad_DF), rad_CF=mean(pts$rad_CF), rad_BF=mean(pts$rad_BF),
      #       rad_AF=mean(pts$rad_AF),rad_AN=mean(pts$rad_AN))
    }
  }
}
  
i1trans
#why doesnt the point(65, 2) exist? What shoudl the true starting point be?
...
data.frame(trivial_validation_accuracy = mean(imagesval$cloud_label == trivial),
           trivial_test_accuracy = mean(imagestest$cloud_label == trivial))
```

A trivial classifier has about a 37% accuracy on both the validation and test data sets. This kind of classifier will have high average accuracy if the actual images depict mostly cloudless regions. 
...
The farther the spread between the two distributions (least overlap) the better. We expect the distribution of cloud vs no clouds to be different, so NDAI seems to be the 'best' feature for distinguishing between the two. This simplifies to a binary classification problem; similar to a decision stump, we can use this feature to train our classifier since it has small error (overlap of histograms). We can asses this error directly in ROC curves, where we compare true positive, and false positive rates. 
```{r}
...
#Add decision Line. Saying the means are far apart is good but we need to add mrpe. Maybe a decision stump, why it's enough to use that feature as a classifier
```
