...
ggplot(just_errors)+
  ...
  scale_fill_manual(name="Error?",values=c(Errors="red", Overall="blue"))
```
The distribution of NDAI values is substantially different among values where our model misclassifies than in the general test set. It appears that our model fails at much higher rates with higher NDAI values. It would be valuable to consult with experts about the possible influence of NDAI on certain regions where our model has a relatively high rate failure. The distributions for SD and CORR are similar. 
