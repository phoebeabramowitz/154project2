...
CVmodel_accuracy <- function(classifier, data, K, loss, featstrain, labeltrain){
  ...
  for (i in 1:K){
    ...
    #knn
    if (classifier=="knn"){
      datclassed <- knn(train_dat[,4:6], val_dat[,4:6],train_dat$cloud_label,k=8)
    }
...
plot(log_perf, colorize=TRUE, lwd=2, main="Logistic ROC curve")
```
...
points(x=0.164,y=0.95,pch=16,cex=2)
```
