...
ggplot(new1, aes(x=NDAI, fill=cloud_label)) + geom_histogram(alpha=0.2, bins=50) + theme_classic() + ggtitle("Histogram of NDAI count based on cloud label: image1") 
...
angles_train <- imagestrain %>%
  select(c("rad_DF", "rad_CF", "rad_BF", "rad_AF" ,"rad_AN"))

pca_angles <- prcomp(angles_train)
