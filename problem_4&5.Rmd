...
ggplot(just_errors)+
  geom_histogram(aes(x=NDAI),fill="red", bins=50)+
...
ggplot(just_errors)+
  geom_histogram(aes(x=CORR),fill="violet", bins=50)+
  aes(y=stat(count)/sum(stat(count))) + 
  scale_y_continuous(labels = scales::percent)+
...
ggplot(just_errors)+
  geom_histogram(aes(x=NDAI),fill="red",alpha=0.3, bins=50)+
  aes(y=stat(count)/sum(stat(count))) + 
  geom_histogram(data=imagestest,aes(x=NDAI),fill="blue", alpha=0.3, bins=50)+
  scale_y_continuous(labels = scales::percent)+
<<<<<<< HEAD
  ylab("Percentage of Erroneous Points in this NDAI Bin")+
...
>>>>>>> 7b54e8b19f61716126a066970e16acaf16eab086
```
