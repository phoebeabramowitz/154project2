...
First, we show the ROC curve to demonstrate the nature of the misclassification errors.
```{r}
...
```
The images above show that the vast majority of the area It's clear from the iamges above that errors are clustered in certain clouds or cloudless regions. 
...
It's possible that our model will work more poorly on future data with expert labels, if there factors in that data that cause the cloudiness of a region to have a different relationship with our features than it has in the three images we trained our model on. Ideally, we would train our model 
