...
CVgeneric <- function(classifier, data, k, loss, splitter=split2){
  ...
  for (i in 1:k){
    ...
    #lda/qda 
    # need to do another 2 else if's, and split up lda and qda as strings the way i have knn and glm
    else{
      formula <- as.formula(cloud_label ~ NDAI + CORR + SD)
...
#split2
CVgeneric(classifier=knn, data=tiny_image_list, k=10, loss=zero_one_loss, splitter=split2)
#split1
CVgeneric(classifier="knn", data=tiny_image_frame, k=10, loss=zero_one_loss, splitter=split1)
CVgeneric(classifier="glm", data=tiny_image_frame, k=10, loss=zero_one_loss, splitter=split1)
...
# Questions for office hours:
 ...
 # - we're getting weirdly high accuracies, and i'm not sure why that is.. 
 #   (could it b/c of the subset of data we're testing this on?)

# set.seed(12345678)
# ### Here's a problem
# debug <- super_pixelize(tiny_image_list)
# ###
# datatrain <- split_data$train
# folds <- createFolds(datatrain$cloud_label, k = k)
# fold_loss <- rep(0,k)
...
#   datclassed <- rep(0, length(first_log.pred))
#   datclassed[model.pred >= 0.5] = 1
#   fold_loss[i] <- zero_one_loss(val_dat$cloud_label, datclassed) 
# }  
# accuracy <- mean(fold_loss)
```
...
```
# need to reconcile the split functions. split1 needs the dataframe, while split2 needs the list. both of them are giving very low accuracies
