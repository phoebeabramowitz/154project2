...
CVgeneric <- function(classifier, data, k, loss, splitter=split2){
  ...
  for(i in 1:k){
    ...
    if(classifier==knn){
      datclassed<- classifier(train_dat)
      for (i in (1:length(Xnames))){ 
      currFormula <- as.formula(paste("log(SalePrice+1)", "~", paste(Xnames[1:i], 
                                                                      collapse = "+"), 
                                      sep = ""))
      modelList[[i]] <- lm(currFormula, AmesTinyTrain)
    }
      fold_loss[i] <- loss(val_dat$cloud_label, datclassed)
    }
    else{
      fold_loss[i] <- loss(val_dat$cloud_label,classifier(train_dat))
    }
  }
...
tiny_image_list <- rbind(image1[1:3000,],image2[1:3000,],image3[1:3000,])
CVgeneric(classifier=,data=tiny_image_list, k=10, loss=, splitter=split1)
