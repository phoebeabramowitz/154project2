...
Since we're using CV to choose models, combine Validation and Training Sets  
```{r}
...
For models that only take in ones and zeros, make new column in timagestrain, timagestest cl
...
datclassed <- rep(0, length(model.pred))
datclassed[model.pred >= 0.5] = 1
zero_one_loss(datclassed, timagestest$cl)
...
```{r} 
#CV for both fold methods, across folds
CVmodel_accuracy("knn",timagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
CVmodel_accuracy("knn",imagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
#Test Accuracy- Untransformed images have slightly better CV accruacy but lower test accuracy
...
```{r} 
#CV for both fold methods, across folds
CVmodel_accuracy("lda",timagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
CVmodel_accuracy("lda",imagestrain,10,loss=zero_one_loss,
                 c("NDAI","CORR","SD"),"cloud_label")
