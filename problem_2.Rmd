...
CVgeneric <- function(classifier, features, labels, k, loss){
  ...
  testSize <- floor(nrow(grouped)*0.1)
  testIndex <- sample(seq_len(nrow(grouped)), size = testSize) 
  #these are diff than before, added underscore to name
  images_train <- grouped[-testIndex, ]
  images_test <- grouped[testIndex, ]
  #divide data into k folds 
  library(caret)
  folds <- createFolds(AmesTinyTrain$SalePrice, k = k)
}
